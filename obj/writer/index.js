...
function parent_alarm0() {
	...
	while (this.stringpos < this.originalstring.length && advance > 0) {
		...
		if (this.ch === "^") {
			...
			} else if (this.ch === "E" || this.ch === "F" || this.ch === "M" || this.ch === "T" || this.ch === "*") {
				this.stringpos++;
			}
	...
	for (let n = 1; n <= this.stringpos; n++)
	{
		...
		else if (this.ch == "\\")
		{
			...
			else if (this.ch == ">")
			{
				...
				else
				{
						...
						if (this.myfont == fnt_ja_comicsans_big)
								myx += 11;
				}
		...
		else
		{
				...
				if (global.language == "en")
				{
						if (global.typer == 18)
						{
								if (this.myletter == "l" || this.myletter == "i")
										letterx += 2;
				...
				{
						if (this.vtext && (this.myfont == fnt_ja_papyrus || this.myfont == fnt_ja_papyrus_btl))
						{
								if (myy == writingy && (this.myletter == "「" || this.myletter == "『"))
										myy -= round((string_width(this.myletter) / 2) * this.htextscale * halfscale);
						}
						else if (this.myfont == fnt_ja_maintext || this.myfont == fnt_ja_main)
						{
								var unit = this.htextscale * halfscale;
								
								if (this.myfont == fnt_ja_main)
										unit *= 2;
				...
				if (global.language == "en")
				{
						...
						else if (this.myfont == fnt_papyrus)
						{
								...
								if (this.myletter == "O" || this.myletter == "W")
										letterx += 2;
				...
				else if (global.language == "ja")
				{
						if (this.vtext)
						{
								myy += round(string_width(this.myletter) * this.htextscale * halfscale);
						}
						...
						{
								if (this.myfont == fnt_ja_comicsans || this.myfont == fnt_ja_comicsans_big)
										letterx -= floor(this.spacing * 0.3);
								else
										letterx -= floor(this.spacing * 0.4);
						}
				}
