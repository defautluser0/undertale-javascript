...
function create() {
  ...
  if (fun === 1) {
    ...
    if (month == 12 || month == 1 || month == 2) weather = 1;

    if (month == 3 || month == 4 || month == 5) weather = 2;

    if (month == 6 || month == 7 || month == 8) weather = 3;

    if (month == 9 || month == 10 || month == 11) weather = 4;
  }
...
function roomEnd() {
  if (this.fun === 1) {
    if (this.harp !== 0) caster_free(this.harp);

    if (this.weathermusic !== 0) caster_free(this.weathermusic);
  }
}
...
function draw() {
  ...
  if (this.fun === 1 && this.harp === 0) {
    ...
    if (this.weather === 1) this.weathermusic = caster_load(mus_options_winter);

    if (this.weather === 2 || this.weather === 4)
      this.weathermusic = caster_load(mus_options_fall);

    if (this.weather === 3) this.weathermusic = caster_load(mus_options_summer);
  }
  ...
  if (this.weather === 1) {
    ...
    draw_text_transformed(
      220 + Math.sin(this.siner / 12),
      120 + Math.cos(this.siner / 12),
  ...
  if (this.weather === 3) {
    ...
    draw_sprite_ext(
      ...
      250,
      225,
      2 + Math.sin(this.siner / 15) * (0.2 + this.extreme / 900),
      2 - Math.sin(this.siner / 15) * (0.2 + this.extreme / 900),
      0,
      c_white,
      1
    ...
    draw_circle(
      258 + Math.cos(this.siner / 18) * 6,
      40 + Math.sin(this.siner / 18) * 6,
      28 + Math.sin(this.siner / 6) * 4,
      0
    );
    draw_set_color(c_gray);
    draw_text_transformed(
  ...
  if (this.menu !== 0) draw_set_color(c_white);
  else draw_set_color(c_yellow);

  draw_text(20, 40, "EXIT");

  if (this.menu != 1) draw_set_color(c_white);
  else draw_set_color(c_yellow);
  ...
  if (this.menu != 2) draw_set_color(c_white);
  else draw_set_color(c_yellow);

  draw_text(20, 100, "IMPORT .INI");

  if (this.menu != 3) draw_set_color(c_white);
  else draw_set_color(c_yellow);

  draw_text(20, 130, "EXPORT .INIS");

  if (this.menu != 4) draw_set_color(c_white);
  else draw_set_color(c_yellow);

  draw_text(20, 160, "IMPORT file0/file9");

  if (this.menu != 5) draw_set_color(c_white);
  else draw_set_color(c_yellow);
