...
import { c_white, playingSounds, rooms } from "/imports/assets.js";
 ...
 */
export function audio_sound_pitch(index, pitch) {
  const id = index._sounds[0]?._id;
  index.rate(pitch, id);
}
 ...
 */
export function audio_sound_gain(index, volume, time = 0) {
  const id = index._sounds[0]?._id;
  if (id !== null) {
 ...
 * @returns {boolean}
 */
export function audio_is_playing(index) {
 ...
 * @param {Real} currentsong this will not be used on engine release
 */
export function audio_stop_sound(index, currentsong) {
  const id = index._sounds[0]?._id;
 ...
 * @param {Real} second The font variable to use. Due to javascript limitations I have to separate fonts from each other or else the font drawer will get confused and draw the wrong font
 * @returns {void}
 */
 ...
 * @param {Real} second The font variable to set. Due to javascript limitations I have to separate fonts from each other or else the font drawer will get confused and draw the wrong font
 */
 ...
 * @param {Room} index The index of the room to go to.
 */
 ...
 * @param {Obj} obj The object to get the number of instances of, or the string "all" to count all instances
 * @returns {Real}
 */
export function instance_number(obj) {
  try {
    ...
    if (typeof obj !== "object") {
      if (typeof obj === "string" && obj === "all") {
        return instances.size;
      } else {
        throw new Error(
          "instance_number(" + obj + "): " + obj + "is not a valid object."
        );
      }
    }

    for (const [, list] of instances.entries()) {
 ...
 * @param {Real} returnImg Engine exclusive, for returning the HTMLImageElement, for width and height checking and stuff.
 * @returns {void}
 */
...
export function ini_export() {
  ...
  const iniText = ini_parse(ini_data);
 ...
 * @returns {void}
 */
export function audio_pause_sound(index, id = null, currentsong = 0) {
  id = id ?? index._sounds[0]?._id;
    ...
    if (perf === true) {
      ...
      if (!newInst)
        throw new Error(
          "instance_change: " +
            obj +
            ".create is undefined/doesnt return properly"
        );
...
export function draw_tile(tileset, tiledata, frame, x, y) {
  ...
  const cached = loadImageCached(key, tileCache);
  if (!cached.loaded) return;
 ...
 * @returns {boolean}
 */
export function file_delete(fname) {
  try {
    if (localStorage.getItem(fname) !== null) {
      localStorage.setItem(fname, null);
      return true;
    } else {
      console.warn(`File ${fname} doesnt exist.`);
      return true;
    }
  } catch (e) {
    console.error(e);
